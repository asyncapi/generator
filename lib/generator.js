      ...
      walker.on('file', async (root, stats, next) => {
        ...
        try {
          if (stats.name.includes('$$channel$$')) {
            ...
            didSeperateIntoFiles = true;
          } else if (stats.name.includes('$$scheme$$')) {
            // this file should be handled for each in asyncapi.components.schemas
            await this.generateSeperateFiles(asyncapiDocument, asyncapiDocument.components().schemas(), 'scheme', stats.name, root);
            didSeperateIntoFiles = true;
          }
