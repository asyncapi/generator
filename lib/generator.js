...
class Generator {
  ...
  generateDirectoryStructure(asyncapiDocument) {
    const fileNamesForSeperation = {
      channel: asyncapiDocument.channels(),
      message: asyncapiDocument.components().messages(),
      securityScheme: asyncapiDocument.components().securitySchemes(),
      schema: asyncapiDocument.components().schemas(),
      parameter: asyncapiDocument.components().parameters(),
      messageTrait: asyncapiDocument.components().messageTraits()
    };
    ...
    return new Promise((resolve, reject) => {
      ...
      walker.on('file', async (root, stats, next) => {
        try {
          ...
          for (const prop in fileNamesForSeperation) {
            if (Object.prototype.hasOwnProperty.call(fileNamesForSeperation, prop)) {
              if (stats.name.includes(`$$${prop}$$`)) {
                await this.generateSeperateFiles(asyncapiDocument, fileNamesForSeperation[prop], prop, stats.name, root);
