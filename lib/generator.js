...
const generateFile = options => new Promise((resolve, reject) => {
  ...
  if (isFileIgnored(file_name, template_config.ignore)) {
 ...
 */
function isFileIgnored (file_name, ignoredFiles) {
  if (!file_name || !ignoredFiles) {
    return false;
  }

  const isIgnored = function(globExp) {
    return minimatch(file_name, globExp);
  };

  return ignoredFiles.some(isIgnored);
}

function resolveTemplateConfig(templates_dir) {
  try {
    const config_path = path.resolve(templates_dir, "config");
