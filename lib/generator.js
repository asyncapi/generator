...
class Generator {
   ...
   * @param {Boolean} [options.forceInstall=false] Force the installation of the template and its dependencies.
   */
  constructor(templateName, targetDir, { templateParams = {}, entrypoint, noOverwriteGlobs, disabledHooks, output = 'fs', forceWrite = false, forceInstall = false } = {}) {
  ...
  async generate(asyncapiDocument) {
    ...
    try {
      ...
      const { name: templatePkgName, path: templatePkgPath } = await this.installTemplate(this.forceInstall);
      this.templateDir = path.resolve(ROOT_DIR, templatePkgPath);
      ...
      if (!force) {
        try {
          ...
          if (isFileSystemPath(this.templateName)) {
            const pkg = require(path.resolve(this.templateName, 'package.json'));
            installedPkg = require(path.resolve(DEFAULT_TEMPLATES_DIR, pkg.name, 'package.json'));
            if (installedPkg.name !== pkg.name) throw new Error('Package not installed');
            ...
            if (await isLocalTemplate(templatePath)) {
              ...
              console.warn(`WARNING: this template is already installed and pointing to your filesystem at ${resolvedLink}.`);
            }
            installedPkg = require(path.resolve(templatePath, 'package.json'));
          }

          return resolve({
            name: installedPkg.name,
            version: installedPkg.version,
            path: path.resolve(DEFAULT_TEMPLATES_DIR, this.templateName),
          });
        } catch (e) {
