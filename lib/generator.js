...
class Generator {
  ...
  /**
   * Generates all the files for each security scheme.
   *
   * @private
   * @param  {AsyncAPIDocument} asyncapiDocument AsyncAPI document to use as the source.
  ...
  async generateSecuritySchemeFile(asyncapiDocument, securitySchemeName, fileName, baseDir) {
    try {
      ...
      const content = await this.renderFile(asyncapiDocument, path.resolve(baseDir, fileName), {
        securitySchemeName,
        securityScheme: asyncapiDocument.components().securityScheme(securitySchemeName),
      });
   ...
   * @return {Promise}
   */
  renderString(asyncapiDocument, templateString, path, extraTemplateData = {}) {
    return new Promise((resolve, reject) => {
