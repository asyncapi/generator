  ...
  installTemplate(force = false) {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        //trying to figure out if this is the right way to access the name of added package https://github.com/npm/arborist/issues/243
        const addResult = arb[Symbol.for('resolvedAdd')];
        if (!addResult) reject('Unable to resolve the name of added package. It was most probably not added to node_modules successfully');
        
        const packageName = addResult[0].name;
        
        return resolve({
          name: packageName,
          path: installResult.children.get(packageName).path,
        });
      } catch (err) {
        reject(err);
      }
