 ...
 * @return {Promise}
 */
const renderFile = (filePath, data, nunjucks) => new Promise((resolve, reject) => {
  fs.readFile(filePath, 'utf8', (err, content) => {
    if (err) return reject(err);

    try {
      nunjucks.renderString(content, data, (err, rendered) => {
        if (err) return reject(err);
        resolve(rendered);
      });
    } catch (e) {
...
const generateFile = options => new Promise((resolve, reject) => {
  ...
  if (isFileNonRenderable(file_name, template_config.nonRenderableFiles)) {
    ...
    renderFile(path.resolve(root, file_name), data, options.nunjucks)
      .then((parsed_content) => {
        const template_path = path.relative(templates_dir, path.resolve(root, file_name));
        const generated_path = path.resolve(target_dir, template_path);
...
const generateTopicFile = (config, topic, topicName) => new Promise((resolve, reject) => {
  fs.readFile(path.join(config.root, config.file_name), 'utf8', (err, data) => {
    ...
    const content = config.nunjucks.renderString(data.toString(), {
...
const generateDirectoryStructure = config => new Promise((resolve, reject) => {
  ...
  walker.on('file', async (root, stats, next) => {
    try {
      if (stats.name.includes('$$topic$$')) {
        ...
        if (!file_path.startsWith(`${PARTIALS_DIRNAME}${path.sep}`) && !file_path.startsWith(`${FILTERS_DIRNAME}${path.sep}`)) {
  ...
  walker.on('directory', async (root, stats, next) => {
    try {
      ...
      if (stats.name !== PARTIALS_DIRNAME && stats.name !== FILTERS_DIRNAME) xfs.mkdirpSync(dir_path);
      next();
    } catch (e) {
      reject(e);
    }
...
const registerHelpers = config => new Promise((resolve, reject) => {
  const helpers_dir = path.resolve(config.templates, FILTERS_DIRNAME);
...
const initNunjucks = config => {
  const nunjucks = new Nunjucks.Environment(new Nunjucks.FileSystemLoader(config.templates));
  nunjucks.addFilter('log', console.log);
  config.nunjucks = nunjucks;
};

const parseAndApplyDefaults = config => new Promise((resolve, reject) => {
  parse(config.asyncapi).then(parsed => {
    config.asyncapi = parsed;
    ...
    _.defaultsDeep(config, {
      package: {
        name: _.kebabCase(config.asyncapi.info().title())
      },
...
generator.generate = config => new Promise((resolve, reject) => {
  if (config.asyncapi) {
    config.asyncapi = fs.readFileSync(config.asyncapi, 'utf-8');
  }

  parseAndApplyDefaults(config)
    .then((cfg) => {
...
generator.generateTemplateFile = ({
  ...
  .then((cfg) => {
    async function start () {
      initNunjucks(cfg);
      await registerHelpers(cfg);
      return renderFile(path.resolve(templatesDir, template, file), cfg, cfg.nunjucks);
    }
