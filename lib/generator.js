...
class Generator {
   ...
   * @param {String} [options.mapBaseUrlToFolder]       Optional parameter to map schema references from URL to a local base folder e.g. https://schema.example.com/crm/:./test/docs/.. The delimiter between URL and folder is a ':'.
   */
  ...
  async generateFromString(asyncapiString, parserOptions = {}) {
    ...
    if (Object.keys(this.mapBaseUrlToFolder).length > 0) {
      parserOptions.resolve = {resolver: this.getMapBaseUrlToFolderResolver(this.mapBaseUrlToFolder)};
    }
  ...
  */
  ...
  /**
   * Creates a custom resolver that maps mapBaseUrlToFolder.url to mapBaseUrlToFolder.folder
   *
   * @private
   * @param  {string} fileName Name of the file to check against a list of glob patterns.
   * @return {{read(*, *, *): Promise<unknown>, canRead(*): boolean, order: number}}
   */
  getMapBaseUrlToFolderResolver(mapBaseUrlToFolder) {
    ...
    return {
      ...
      read(file, callback, $refs) {
        ...
        return new Promise(((resolve, reject) => {
          const localPath = file.url.replace(baseUrl,baseDir);
          try {
