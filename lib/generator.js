...
class Generator {
   ...
   * @param {String} [options.forceWrite] Force writing of the generated files to given directory even if it is a git repo with unstaged files or not empty dir.
   */
  constructor(templateName, targetDir, { templatesDir, templateParams, entrypoint, noOverwriteGlobs, disabledHooks, output = 'fs', forceWrite } = {}) {
  ...
  async verifyTargetDir(dir) {
    ...
    try {
      ...
      if (isGitRepo) {
        ...
        if (hasModifiedUstagedFiles || hasUntrackedUnstagedFiles) throw new Error(`"${this.targetDir}" is in a git repository with unstaged changes`);
      } else {
        const isDirEmpty = (await readDir(dir)).length === 0;

        if (!isDirEmpty) throw new Error(`"${this.targetDir}" is not an empty directory. You might override your work.`);
      }
    } catch (e) {
      throw e;
    }
