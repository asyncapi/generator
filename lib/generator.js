...
class Generator {
  ...
  constructor(templateName, targetDir, { templatesDir }) {
    ...
    this.resolveTemplateConfig();
  }

  /**
   * Generates files from a given template and an AsyncAPIDocument object.
   *
   * @param  {String} asyncapiDocument AsyncAPIDocument object to use as source.
   ...
   *
   * @param  {String} asyncapiDocument AsyncAPI document to pass to the template.
  ...
  /**
   * Reads the content of a given template file.
   *
   * @param  {String} filePath Path to the file to render. Relative to the template directory.
   * @return {Promise<string, Error>}
   */
  ...
  /**
   * Checks if a given fileName matches a list of Glob patterns
   *
   * @param  {string} fileName Name to check against a list of Glob patterns.
   * @return {boolean}
   */
