...
reactExport.saveRenderedReactContent = async (renderedContent, outputPath) => {
  if (Array.isArray(renderedContent)) {
    return await Promise.all(renderedContent.map(content => saveContentToFile(content, outputPath)));
  }
  return await saveContentToFile(renderedContent, outputPath);
};
 ...
 * @param {String} outputPath Path to the file being rendered.
 */
saveContentToFile = async (renderedContent, outputPath) => {
