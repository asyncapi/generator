 ...
 * @returns {Object|undefined} object representing content of the package.json file with additional field -> pkgPath (absolute path to the package). Read {@link https://raw.githubusercontent.com/sindresorhus/type-fest/main/source/package-json.d.ts|PackageJson} for more detaisl on contents of the package.json object
 */
utils.getTemplateDetails = (name, PACKAGE_JSON_FILENAME) => {
  ...
  if (utils.isFileSystemPath(name)) {
    ...
    log.debug('Remember that your local template must have its own node_modules installed first, "npm install" is not triggered by the generator.');
  } else {
    ...
    if (!pkgPath) {
      ...
      if (pkgPackageJsonPath) {
        log.debug(`${name} not found in local dependencies but found it installed as a global package.`);
