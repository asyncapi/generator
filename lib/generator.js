...
class Generator {
  ...
  async renderAndWriteToFile(asyncapiDocument, templateFilePath, extraTemplateData) {
    ...
    if (this.templateConfig && this.templateConfig.type === 'react') {
      ...
      const permission = renderContent.metadata.permission || 0o200;
      ...
      await writeFile(templateFilePath, content, {
        mode: permission
      });
    } else {
      await writeFile(templateFilePath, renderContent);
    }
  }
  /**
   * Generates a file.
  ...
  renderReact(asyncapiDocument, filePath, extraTemplateData = {}) {
    return AsyncReactSDK.test(filePath);
  }
  ...
  async renderFile(asyncapiDocument, filePath, extraTemplateData = {}) {
    ...
    if (this.templateConfig && this.templateConfig.type === 'react') {
      return this.renderReact(asyncapiDocument, filePath, extraTemplateData);
    }
