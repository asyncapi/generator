...
class Generator {
  ...
  async generate(asyncapiDocument) {
    ...
    if (this.entrypoint) {
      ...
      if (this.output === 'fs') {
        ...
        return this.renderFile(asyncapiDocument, entrypointPath).content;
      }
    }else{
  ...
  async generateSeparateFile(asyncapiDocument, name, component, template, fileName, baseDir) {
    ...
    temp[`${key}Name`] = name;
    temp[key] = component;
    await this.renderAndWriteToFile(asyncapiDocument, path.resolve(baseDir, fileName), temp)
  }
   ...
   */
  async renderAndWriteToFile(asyncapiDocument, templateFilePath, extraTemplateData){
    const renderContent = await this.renderFile(asyncapiDocument, templateFilePath, temp);
    if(this.templateConfig && this.templateConfig.type === 'react'){
      const newFileName = renderContent.metadata.name;
      const permissions = renderContent.metadata.permissions
      const content = renderContent.content;
      if(newFileName && newFileName.trim() !== ""){
        const basepath = path.basename(templateFilePath)
        templateFilePath.replace(basepath, newFileName);
      }
      await writeFile(targetFile, content, {
        mode: permissions
      });
    }else{
      await writeFile(targetFile, renderContent);
    }
  }
  /**
   * Generates a file.
  ...
  async generateFile(asyncapiDocument, fileName, baseDir) {
    ...
    await this.renderAndWriteToFile(asyncapiDocument, sourceFile)
  }
   ...
   * @return {Promise}
   */
  renderNunjucks(asyncapiDocument, tempalteString, extraTemplateData = {}) {
    return new Promise((resolve, reject) => {
      this.nunjucks.renderString(tempalteString, {
  ...
  renderReact(asyncapiDocument, content, extraTemplateData = {}) {
    return AsyncReactSDK.render(tempalteString, content);
  }
  ...
  async renderFile(asyncapiDocument, filePath, extraTemplateData = {}) {
    const tempalteString = await readFile(filePath, 'utf8');
    if(this.templateConfig && this.templateConfig.type === 'react'){
      return this.renderReact(asyncapiDocument, tempalteString, extraTemplateData)
    }else{
      return this.renderNunjucks(asyncapiDocument, tempalteString, extraTemplateData);
    }
  }
