...
class Generator {
  ...
  async generateFromString(asyncapiString, parserOptions = {}) {
    ...
    if (this.mapBaseUrlToFolder.url) {
      parserOptions.resolve = {resolver: this.getMapBaseUrlToFolderResolver()};
    }
  ...
  getMapBaseUrlToFolderResolver() {
    ...
    return {
      ...
      read(file, callback, $refs) {
        const baseUrl = this.mapBaseUrlToFolder.url;
        const baseDir = this.mapBaseUrlToFolder.folder;
