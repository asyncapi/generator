...
/**
 * Check if given template is installed and return details about it
 * 
 * @private
 * @param {String} name name of the template
 * @param {String} PACKAGE_JSON_FILENAME standard name of the package.json file
 * @returns {Object|undefined} object with fields like pkgPath, name, version
 */
utils.getTemplateDetails = (name, PACKAGE_JSON_FILENAME) => {
  ...
  if (utils.isFileSystemPath(name)) {
    ...
    //no we need to look for the template in global location
    ...
    if (!pkgPath) {
      ...
      if (pkgPackageJsonPath) {
        ...
        pkgPath = pkgPackageJsonPath.substring(0, pkgPackageJsonPath.lastIndexOf('/'));
      }
    }
  }
