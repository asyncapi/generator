...
class Generator {
  ...
  installTemplate(force = false) {
    return new Promise(async (resolve, reject) => {
      if (!force) {
        try {
          const pkgPath = resolvePkg(this.templateName);
          const installedPkg = require(path.join(pkgPath, PACKAGE_JSON_FILENAME));
          if (pkgPath && this.debug) console.info(`Template sources taken from ${pkgPath}`);
          return resolve({
