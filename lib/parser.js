...
parser.parse = async (asyncapi, oldOptions, generator) => {
  try {
    const { sanitizeTemplateApiVersion, usesNewAPI, getProperApiDocument, convertOldOptionsToNew } = parser;
    validateInputs(asyncapi,oldOptions,generator);
    let apiVersion = sanitizeTemplateApiVersion(generator.templateConfig.apiVersion);
  // Defaulting to apiVersion v1 to convert it to the Parser-API v1 afterwards.
  if (!usesNewAPI(generator.templateConfig)) {
    apiVersion = 1;
  }
  ...
  const correctDocument = getProperApiDocument(document, generator.templateConfig);
  return {document: correctDocument, diagnostics};
    
  } catch (error) {
    console.errpr("Error parsing AsyncAPI document",error);
    throw error;
  }
};

function validateInputs(asyncapi,oldOptions,generator){
  if (!asyncapi) {
    throw new Error('asyncapi document is required');
  }
  if (!generator || !generator.templateConfig) {
    throw new Error('generator with templateConfig is required');
  }
}
...
parser.usesNewAPI = (templateConfig = {}) => {
  return sanitizeTemplateApiVersion(templateConfig.apiVersion) > 0;
};
