...
const registerHelpers = config => new Promise((resolve, reject) => {
  ...
  walker.on('file', async (root, stats, next) => {
    try {
      ...
      if (typeof mod === 'function') mod(config.nunjucks, _);
      next();
    } catch (e) {
      reject(e);
    }
...
const initNunjucks = config => {
  config.nunjucks = new Nunjucks.Environment(new Nunjucks.FileSystemLoader(config.templates));
};

const parseAndApplyDefaults = config => new Promise((resolve, reject) => {
  try {
    const parsed = parse(config.asyncapi);
    config.asyncapi = beautifier(parsed);
