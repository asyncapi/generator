 ...
 * @param {String} mainPackageName The name of the template that is installed
 * @returns {Object}
 */
utils.beautifyNpmiResult = (result, mainPackageName) => {
  const mainPackageResult = result.filter(arr => arr[0] === mainPackageName);
  const [nameWithVersion, pkgPath] = mainPackageResult[0];
