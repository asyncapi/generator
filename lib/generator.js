...
class Generator {
  ...
  async generateFromString(asyncapiString, asyncApiFileLocation) {
    ...
    try {
      const parsed = await parse(asyncapiString, parseOptions);
  ...
  async generateSeparateFile(asyncapiDocument, name, component, template, fileName, baseDir) {
    try {
      ...
      const newFileName = fileName.replace(`\$\$${template}\$\$`, name);
   ...
   * @return {Promise}
   */
  renderString(asyncapiDocument, templateString, pathToFile, extraTemplateData = {}) {
    return new Promise((resolve, reject) => {
