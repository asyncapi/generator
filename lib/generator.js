...
const templateSource = require('./logMessages').templateSource();
const packVersion = require('./logMessages').templateVersion();
const packageNotAvailable = require('./logMessages').packageNotAvailable();
const npmInstallTrigger = require('./logMessages').npmInstallTrigger;
const InstallationDebugMessage = require('./logMessages').InstallationDebugMessage();
const TemplateSuccessfullyInstalled = require('./logMessages').TemplateSuccessfullyInstalled();
const RelativeSourceFileNotGenerated = require('./logMessages').RelativeSourceFileNotGenerated();
const NotGeneratedConditionalFilesMatched = require('./logMessages').conditionalFilesMatched();
...
class Generator {
    ...
    return new Promise(async (resolve, reject) => {
      if (!force) {
        ...
        try {
          ...
          log.debug(templateSource(pkgPath));
          if (packageVersion) log.debug(packVersion(packageVersion));
          ...
          log.debug(packageNotAvailable(pkgPath), e);
      ...
      log.debug(InstallationDebugMessage(debugMessage));

      if (isFileSystemPath(this.templateName)) log.debug(npmInstallTrigger);
      ...
      try {
        ...
        if (!isFileSystemPath(this.templateName)) log.debug(TemplateSuccessfullyInstalled(packageName, packagePath));
        if (packageVersion) log.debug(packVersion(packageVersion));
  ...
  async generateFile(asyncapiDocument, fileName, baseDir) {
    ...
    if (this.templateConfig.conditionalFiles && this.templateConfig.conditionalFiles[relativeSourceFile]) {
      ...
      if (!source) return log.debug(RelativeSourceFileNotGenerated(relativeSourceFile, this.templateConfig.conditionalFiles[relativeSourceFile].subject));

      if (source) {
        const validate = this.templateConfig.conditionalFiles[relativeSourceFile].validate;
        const valid = validate(source);
        if (!valid) return log.debug(NotGeneratedConditionalFilesMatched(relativeSourceFile));
      }
    }
