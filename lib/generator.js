    ...
    return new Promise(async (resolve, reject) => {
      if (!force) {
        ...
        try {
          ...
          if (packageVersion) log.debug(logMessage.packVersion(packageVersion));
  ...
  async generateFile(asyncapiDocument, fileName, baseDir) {
    ...
    if (this.templateConfig.conditionalFiles && this.templateConfig.conditionalFiles[relativeSourceFile]) {
      ...
      if (source) {
        ...
        if (!valid) return log.debug(logMessage.NotGeneratedConditionalFilesMatched(relativeSourceFile));
      }
    }
