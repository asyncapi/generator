 ...
 */
function registerConfigHooks(hooks, templateDir, templateConfig) {
  const configHooks = templateConfig.hooks;

  if (typeof configHooks !== 'object' || !Object.keys(configHooks).length > 0) return;

  Object.keys(configHooks).forEach(hooksModuleName => {
    const modulePath = resolvePkg(hooksModuleName, { cwd: templateDir });
