...
class Generator {
  ...
  async generateFromString(asyncapiString, parserOptions = {}) {
    ...
    const { document } = await parser.parse(asyncapiString, parserOptions);
  ...
  getAllParameters(asyncapiDocument) {
    ...
    if (usesNewAPI(this.templateConfig)) {
      asyncapiDocument.channels().forEach(channel => {
        channel.parameters().forEach(parameter => {
          parameters.set(parameter.id(), parameter);
        });
      });

      asyncapiDocument.components().channelParameters().forEach(parameter => {
        parameters.set(parameter.id(), parameter);
      });
    } else {
  ...
  async filesGenerationHandler(asyncapiDocument, root, stats, next) {
    ...
    if (usesNewAPI(this.templateConfig)) {
      fileNamesForSeparation = {
        channel: convertCollectionToObject(asyncapiDocument.channels(), 'address'),
        message: convertCollectionToObject(asyncapiDocument.messages(), 'id'),
        securityScheme: convertCollectionToObject(asyncapiDocument.components().securitySchemes(), 'id'),
        schema: convertCollectionToObject(asyncapiDocument.components().schemas(), 'id'),
        objectSchema: convertCollectionToObject(asyncapiDocument.schemas().filter(schema => schema.type() === 'object'), 'id'),
        parameter: convertMapToObject(this.getAllParameters(asyncapiDocument)),
        everySchema: convertCollectionToObject(asyncapiDocument.schemas(), 'id'),
      };
    } else {
      const objectSchema = {};
