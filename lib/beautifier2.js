...
module.exports = (asyncapi) => {
  ...
  _.each(asyncapi.channels, (channel, channelName) => {
    if (channel.publish && channel.publish.message) {
      ...
      if (channel.publish.message.oneOf) _.each(channel.publish.message.oneOf, beautifyMessage);
      asyncapi.publishChannels[channelName] = channel;
    }

    if (channel.subscribe) {
      beautifyMessage(channel.subscribe.message);
      if (channel.subscribe.message.oneOf) _.each(channel.subscribe.message.oneOf, beautifyMessage);
