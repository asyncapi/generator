...
class Generator {
   ...
   * @param {String} templateFilePath Path to the file being rendered.
  ...
  async renderAndWriteToFile(asyncapiDocument, templateFilePath, outputpath, extraTemplateData) {
    ...
    if (isReactTemplate(this.templateConfig)) {
      ...
      if (renderContent.metadata !== undefined) {
        ...
        if (renderContent.metadata.permission !== undefined) {
          permission = renderContent.metadata.permission;
        }
   ...
   * @return {Promise}
   */
  renderNunjucks(asyncapiDocument, tempalteString, filePath, extraTemplateData = {}) {
    return new Promise((resolve, reject) => {
   ...
   */
  async renderReact(asyncapiDocument, filePath, extraTemplateData = {}) {
    filePath = filePath.replace(this.templateContentDir, path.resolve(this.templateDir, TRANSPILED_TEMPLATE_LOCATION));
    return await AsyncReactSDK.renderTemplate(filePath, {
      asyncapi: asyncapiDocument,
      params: this.templateParams,
      originalAsyncAPI: this.originalAsyncAPI,
      ...extraTemplateData
   ...
   * @param {Object} [extraTemplateData] Extra data to pass to the template.
   * @return {Promise<string|import('asyncAPISDK').TemplateRenderResult>}
   */
  async renderFile(asyncapiDocument, filePath, extraTemplateData = {}) {
    if (isReactTemplate(this.templateConfig)) {
      return await this.renderReact(asyncapiDocument, filePath, extraTemplateData);
    }
    const tempalteString = await readFile(filePath, 'utf8');
    return this.renderNunjucks(asyncapiDocument, tempalteString, filePath, extraTemplateData);
  }
   ...
   * @private
   */
  async configureReact() {
    const outputDir = path.resolve(this.templateDir, `./${TRANSPILED_TEMPLATE_LOCATION}`);
    await AsyncReactSDK.transpileFiles(this.templateContentDir, outputDir, {
      recursive: true
    });
  }
