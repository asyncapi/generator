...
class Generator {
  ...
  async filesGenerationHandler(asyncapiDocument, objectMap, root, stats, next) {
    const fileNamesForSeparation = {
      ...
      message: asyncapiDocument.messages() ? asyncapiDocument.messages().reduce((map, message) => {
      ...
      parameter: asyncapiDocument.channels() ? asyncapiDocument.channels().reduce((map, channel) => {
        const channelParameters = channel.parameters() ? channel.parameters().reduce((parameterMap, schema) => {
          parameterMap[schema.uid()] = schema;
          return map;
        }, {}) : {};
        Object.assign(map, channelParameters);
        return map;
      }, {}) : {},
  ...
  async generateFile(asyncapiDocument, fileName, baseDir) {
    ...
    if (this.templateConfig.conditionalFiles && this.templateConfig.conditionalFiles[relativeSourceFile]) {
      const server = this.templateParams.server && asyncapiDocument.servers(this.templateParams.server);
