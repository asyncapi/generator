...
- When defaults aren't available in the AsyncAPI document, users must provide **both** `discriminator_key` and `discriminator_value` when registering handlers
- Providing only one parameter is not supported - you must provide either both or neither
...
- AsyncAPI 3.0 currently supports discriminator values as strings only.
