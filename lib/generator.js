...
class Generator {
  ...
  async generateSeparateFile(asyncapiDocument, name, component, template, fileName, baseDir) {
    try {
      ...
      const beforeFileGenerationHookName = 'generate:changeFilename'
      let filename = name
      if(this.isHookAvailable(beforeFileGenerationHookName)){
        let filenamesFromHooks = await this.launchHook(beforeFileGenerationHookName, {"filename": filename})
  ...
  async launchHook(hookName, hookArguments) {
    ...
    const promises = this.hooks[hookName].map(async (hook) => {
      ...
      var returnValue = await hook(this, hookArguments);
      return returnValue;
    });
